...
export class ZGBrokerService {
  ...
  async initialize(): Promise<void> {
    ...
    if (typeof window === "undefined") {
      throw new Error("0G Broker can only be initialized in browser environment");
    }
    ...
    try {
      ...
      // Use wallet from private key or environment variable
  ...
  async depositFund(amount: string): Promise<void> {
    ...
    try {
      ...
      console.log(`Deposited ${amount} OG tokens`);
    } catch (error) {
  ...
  async retrieveFund(serviceType: string, amount: string): Promise<void> {
    if (!this.isInitialized) await this.initialize();
    
    try {
      await this.broker.ledger.retrieveFund(serviceType, amount);
      console.log(`Retrieved ${amount} OG tokens from ${serviceType}`);
    } catch (error) {
